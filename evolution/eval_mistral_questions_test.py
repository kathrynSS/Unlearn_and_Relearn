...
def find_forget_model_path(forget_cfg_path: str) -> str:
    ...
    if not root.exists():
        raise FileNotFoundError(f"Forgotten model root directory does not exist: {root} (please verify training has been completed)")

    candidates = [p for p in root.iterdir() if p.is_dir()]
    if not candidates:
        raise FileNotFoundError(f"No forgotten model subdirectory found under {root}. Please complete training first.")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        "comparing the base model and the forgotten model on A/B/C/D choices."
    )
    parser.add_argument(
        "--test_path",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional: manually specify the forgotten model subdirectory name (e.g. '70_1e-06_forget'), "
             "overriding the auto-search for the latest subdirectory.",
    )
    parser.add_argument(
        "--forget_adapter_dir",
        type=str,
        default=None,
        help="Optional: if the forgotten result only contains a LoRA adapter (adapter_model.safetensors), "
             "specify the directory path directly (absolute or relative).",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Optional: evaluate only the first N samples for quick testing.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="Optional: output directory. If specified, saves eval_results.json there. "
             "Otherwise, saves to the --forget_adapter_dir directory.",
    )
    ...
    if not required_cols.issubset(df.columns):
        raise ValueError(f"questions_test.xlsx must contain columns: {required_cols}, found: {set(df.columns)}")

    if args.max_samples is not None:
        df = df.head(args.max_samples)
        print(f"Evaluating first {len(df)} samples.")
    else:
    ...
    for idx, row in df.iterrows():
        ...
        print(f"[Base model] Sample {idx + 1}/{len(df)}: pred={choice}, GT={gt_letters[idx]} (raw: {gt_full[idx]})")
    ...
    if args.forget_adapter_dir is not None:
        ...
        if is_adapter_only:
            ...
            print(f"\nLoading forgotten full model directory: {forget_model_path}")
            forget_model_path_str = str(forget_model_path)
    ...
    for idx, row in df.iterrows():
        ...
        print(f"[Forgotten model] Sample {idx + 1}/{len(df)}: pred={choice}, GT={gt_letters[idx]} (raw: {gt_full[idx]})")
    ...
    print("\n================= Accuracy Summary =================")
    print(f"Base model accuracy: {base_acc * 100:.2f}% ({base_correct}/{len(df)})")
